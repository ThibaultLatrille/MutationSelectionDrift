...
def create_experiment(prefix, name, sample, replicate, tree_name, cds_list):
    ...
    for rep in range(replicate):
        experiment = prefix + "_{0}_{1}_{2}_Sample{3}_Replicates{4}_Id{5}".format(name, tree_name, cds_list, sample, replicate, rep)
        ...
        with open(run_file, 'w') as w:
            ...
            run_str = 'snakemake -j {0}'.format(replicate)
            w.write(run_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--tree', required=False, type=str, default="rootedtree.lht.nhx", dest="tree")
    ...
    create_experiment(args.prefix, args.name, args.sample, args.replicate, args.tree, args.cds)
