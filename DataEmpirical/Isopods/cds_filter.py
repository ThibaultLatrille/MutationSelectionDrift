...
for gblock in os.listdir(folder_path + '/fasta'):
    block_path = folder_path + '/fasta/' + gblock
    filtered_seq = [(record.name.split("_")[0], str(record.seq).upper()) for record in SeqIO.parse(block_path, "fasta")]
    assert(len(set([len(s[1]) for s in filtered_seq])) == 1)
    ali_file = open(block_path.replace("/fasta", "/singlegene_alignments").replace(".cds.fa.prot2nucl.prank.fas-gb.renamed.njpl", ".ali"), 'w')
