...
def plot_trace(input_tree, input_trace, output_plot, burn_in):
    traces = dict()
    filenames = []
    ...
    for param, traces_param in traces.items():
        ...
        for filename, param_trace in sorted(traces_param.items(), key=lambda x: x[0]):
            ...
            if param[0] == "*":
                ...
                node_name = "_".join(param.split("_")[1:])
    ...
    for arg in ["LeafTheta", "BranchdNdS", "LogNodeMutRate", "LogBranchMutRate",
        ...
        if len(axis_dict) > 1:
            for filename, axis in axis_dict.items():
                ...
                tree.render("{0}/nhx.{1}.{2}.png".format(output_plot, arg, filename), tree_style=ts, w=1080, h=1440)

            plot_correlation('{0}/correlation.{1}.png'.format(output_plot, arg), axis_dict, err_dict, bl)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-s', '--tree', required=True, type=str, dest="tree")
    ...
    plot_trace(args.tree, args.trace, args.output, args.burn_in)
